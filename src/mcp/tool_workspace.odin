...
_RULES :: []RuleCatalogEntry{
    ...
    {"C031", "correctness", "style_panic_on_error",
     "panic() on expected runtime failure — consider returning an error value instead (INFO)",
     "Replace panic(...) with a proper error return so callers can handle the failure", true},
    {"C034", "style", "style_unused_blank_index",
     "for v, _ in collection — blank index is unnecessary; use for v in (INFO, auto-fix)",
     "Remove the blank index: change 'for v, _ in coll' to 'for v in coll'", true},
    {"C037", "style", "style_trailing_void_return",
     "trailing 'return' at end of void procedure is redundant (INFO, auto-fix)",
     "Remove the trailing 'return'", true},
