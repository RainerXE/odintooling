...
// Exposes get_dna_context, get_impact_radius, find_allocators, find_all_references,
// rename_symbol, run_lint_denoise, get_symbol, export_symbols, get_callers,
// get_callees, and search_symbols to AI agents.
package mcp_server
...
make_get_dna_context_tool :: proc() -> mcp.Tool {
    return mcp.Tool{
        ...
        simple_handler = _get_dna_context_handler,
    }
}

@(private="file")
_get_dna_context_handler :: proc(params: json.Value, allocator: runtime.Allocator) -> mcp.Tool_Result {
    ...
    if !ok {
        return mcp.tool_error(.Not_Found,
            fmt.aprintf("graph db not found at %s — run --export-symbols first", db_path, allocator = allocator),
            allocator)
    }
    ...
    if !found {
        return mcp.tool_error(.Not_Found,
            fmt.aprintf("proc '%s' not found in graph", proc_name, allocator = allocator),
            allocator)
    }
    ...
    return mcp.tool_ok(strings.clone(strings.to_string(sb), allocator), allocator)
}
...
make_get_impact_radius_tool :: proc() -> mcp.Tool {
    return mcp.Tool{
        defn = mcp.ToolDefinition{
        ...
        simple_handler = _get_impact_radius_handler,
    }
}

@(private="file")
_get_impact_radius_handler :: proc(params: json.Value, allocator: runtime.Allocator) -> mcp.Tool_Result {
    proc_name, err := _extract_string_param(params, "proc_name")
    if err != "" { return mcp.tool_error(.Invalid_Params, err, allocator) }
    ...
    if !ok {
        return mcp.tool_error(.Not_Found,
            fmt.aprintf("graph db not found at %s", db_path, allocator = allocator),
            allocator)
    }
    defer core.graph_close(db)
...
make_find_allocators_tool :: proc() -> mcp.Tool {
    return mcp.Tool{
        defn = mcp.ToolDefinition{
        ...
        simple_handler = _find_allocators_handler,
    }
}

@(private="file")
_find_allocators_handler :: proc(params: json.Value, allocator: runtime.Allocator) -> mcp.Tool_Result {
    db_path := _graph_opt_string(params, "db_path", core.GRAPH_DB_PATH)
...
make_find_all_references_tool :: proc() -> mcp.Tool {
    return mcp.Tool{
        defn = mcp.ToolDefinition{
        ...
        simple_handler = _find_all_references_handler,
    }
}

@(private="file")
_find_all_references_handler :: proc(params: json.Value, allocator: runtime.Allocator) -> mcp.Tool_Result {
...
make_rename_symbol_tool :: proc() -> mcp.Tool {
    return mcp.Tool{
        defn = mcp.ToolDefinition{
            name        = "rename_symbol",
        ...
        simple_handler = _rename_symbol_handler,
    }
}

@(private="file")
_rename_symbol_handler :: proc(params: json.Value, allocator: runtime.Allocator) -> mcp.Tool_Result {
    old_name, err1 := _extract_string_param(params, "old_name")
    if err1 != "" { return mcp.tool_error(.Invalid_Params, err1, allocator) }
    new_name, err2 := _extract_string_param(params, "new_name")
    if err2 != "" { return mcp.tool_error(.Invalid_Params, err2, allocator) }
    db_path := _graph_opt_string(params, "db_path", core.GRAPH_DB_PATH)

    if old_name == new_name {
        return mcp.tool_ok(`{"edits":[],"edit_count":0,"note":"old and new names are identical"}`, allocator)
    }
    ...
    if len(locs) == 0 {
        return mcp.tool_ok(
            fmt.aprintf(`{{"edits":[],"edit_count":0,"note":"symbol '%s' not found in graph"}`, old_name, allocator = allocator),
            allocator)
    }
...
// of the first occurrence of `token` as a whole word.
@(private="file")
...
make_run_lint_denoise_tool :: proc() -> mcp.Tool {
    return mcp.Tool{
        defn = mcp.ToolDefinition{
        ...
        simple_handler = _run_lint_denoise_handler,
    }
}

@(private="file")
_run_lint_denoise_handler :: proc(params: json.Value, allocator: runtime.Allocator) -> mcp.Tool_Result {
...
make_get_symbol_tool :: proc() -> mcp.Tool {
    return mcp.Tool{
        defn = mcp.ToolDefinition{
            name        = "get_symbol",
        ...
        simple_handler = _get_symbol_handler,
    }
}

@(private="file")
_get_symbol_handler :: proc(params: json.Value, allocator: runtime.Allocator) -> mcp.Tool_Result {
    symbol, err := _extract_string_param(params, "symbol")
    if err != "" { return mcp.tool_error(.Invalid_Params, err, allocator) }
    db_path := _graph_opt_string(params, "db_path", core.GRAPH_DB_PATH)
    ...
    if !found {
        return mcp.tool_error(.Not_Found,
            fmt.aprintf("symbol '%s' not found in graph", symbol, allocator = allocator),
            allocator)
    }
    defer _free_node(&node)
    ...
    result := fmt.aprintf(
        ...
        _gj(node.signature), _gj(role), violations, allocator = allocator)
    return mcp.tool_ok(result, allocator)
}
...
make_export_symbols_tool :: proc() -> mcp.Tool {
    return mcp.Tool{
        defn = mcp.ToolDefinition{
            name        = "export_symbols",
        ...
        simple_handler = _export_symbols_handler,
    }
}

@(private="file")
_export_symbols_handler :: proc(params: json.Value, allocator: runtime.Allocator) -> mcp.Tool_Result {
    ...
    if !r.ok {
        return mcp.tool_error(.Internal, "export_symbols pipeline failed", allocator)
    }
    ...
    result := fmt.aprintf(
        ...
        cached_str, _gj(r.db_path), _gj(r.symbols_path), allocator = allocator)
    return mcp.tool_ok(result, allocator)
}
...
make_get_callers_tool :: proc() -> mcp.Tool {
    return mcp.Tool{
        defn = mcp.ToolDefinition{
            name        = "get_callers",
        ...
        simple_handler = _get_callers_handler,
    }
}

@(private="file")
_get_callers_handler :: proc(params: json.Value, allocator: runtime.Allocator) -> mcp.Tool_Result {
    proc_name, err := _extract_string_param(params, "proc_name")
    if err != "" { return mcp.tool_error(.Invalid_Params, err, allocator) }
    db_path := _graph_opt_string(params, "db_path", core.GRAPH_DB_PATH)
...
make_get_callees_tool :: proc() -> mcp.Tool {
    return mcp.Tool{
        defn = mcp.ToolDefinition{
            name        = "get_callees",
        ...
        simple_handler = _get_callees_handler,
    }
}

@(private="file")
_get_callees_handler :: proc(params: json.Value, allocator: runtime.Allocator) -> mcp.Tool_Result {
    proc_name, err := _extract_string_param(params, "proc_name")
    if err != "" { return mcp.tool_error(.Invalid_Params, err, allocator) }
    db_path := _graph_opt_string(params, "db_path", core.GRAPH_DB_PATH)
...
make_search_symbols_tool :: proc() -> mcp.Tool {
    return mcp.Tool{
        defn = mcp.ToolDefinition{
            name        = "search_symbols",
        ...
        simple_handler = _search_symbols_handler,
    }
}

@(private="file")
_search_symbols_handler :: proc(params: json.Value, allocator: runtime.Allocator) -> mcp.Tool_Result {
    query, err := _extract_string_param(params, "query")
    if err != "" { return mcp.tool_error(.Invalid_Params, err, allocator) }
    if len(query) == 0 {
        return mcp.tool_ok(`{"symbols":[],"count":0}`, allocator)
    }
...
// _gj returns a JSON-quoted string.
@(private="file")
