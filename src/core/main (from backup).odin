...
import "core:os"
import "core:strings"
import lsp "src/core/lsp_server"
// LSP integration is now in the core package
...
main :: proc() {
    ...
    if len(args) > 2 {
        if args[2] == "--ast" {
            ...
            lsp_mode = true
            fmt.println("Running in LSP mode")
        }
    }
    ...
    if lsp_mode {
        // Check if we should run as server or CLI
        if len(args) > 3 && args[3] == "--server" {
            lsp.runLSPServer()
        } else {
            runLSPCLI(file_path)
        }
