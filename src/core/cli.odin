...
print_help :: proc() {
    ...
    fmt.println("  C021  correctness  Go-style fmt.Println/Printf/Sprintf call (enable-go-migration)")
    fmt.println("  C022  correctness  Go-style 'for i,v := range' loop (enable-go-migration)")
    fmt.println("  C023  correctness  C-style '*ptr' dereference — use 'ptr^' (enable-go-migration)")
    fmt.println("  C025  correctness  append(slice, v) missing address-of — use append(&slice, v) (enable-go-migration)")
    fmt.println("  C029  correctness  stdlib alloc (strings.split/clone/join, fmt.aprintf, os.read_entire_file…) without defer delete (stdlib_safety)")
    fmt.println("  C031  correctness  panic() on expected runtime failure — consider returning an error instead (INFO)")
    fmt.println("  C033  correctness  strings.builder_make() without defer strings.builder_destroy (stdlib_safety)")
    fmt.println("  C034  style        for v, _ in collection — blank index unnecessary; use for v in (INFO, auto-fix)")
    fmt.println("  C037  style        trailing return at end of void procedure is redundant (INFO, auto-fix)")
    ...
    fmt.println("  olt init               Create olt.toml in current directory (runs 'olt setup' first if needed)")
    fmt.println("  olt setup              Full system setup: OLS detection, install olt + symlinks")
    fmt.println("  olt mcp                Start MCP server (stdio JSON-RPC for Claude Code etc.)")
    fmt.println("  olt lsp                Start LSP proxy (wraps OLS, injects olt diagnostics)")
    fmt.println()
    fmt.println("Options:")
    fmt.println("  --init                 Alias for 'olt setup' (backward compat)")
    fmt.println("  --install              Re-run install step only (after a rebuild)")
...
print_list_rules :: proc() {
    ...
    fmt.println("C101\tcorrectness\tcontext.allocator assigned without defer restore")
    fmt.println("C021\tcorrectness\tGo-style fmt.Println/Printf/Sprintf call (go_migration domain)")
    fmt.println("C022\tcorrectness\tGo-style 'for i,v := range' loop (go_migration domain)")
    fmt.println("C023\tcorrectness\tC-style '*ptr' dereference — use 'ptr^' (go_migration domain)")
    fmt.println("C025\tcorrectness\tappend(slice, v) missing address-of — use append(&slice, v) (go_migration domain)")
    fmt.println("C029\tcorrectness\tstdlib alloc without defer delete (stdlib_safety domain)")
    fmt.println("C031\tcorrectness\tpanic() on expected runtime failure — prefer error return (INFO)")
    fmt.println("C033\tcorrectness\tstrings.builder_make() without defer strings.builder_destroy (stdlib_safety domain)")
    fmt.println("C034\tstyle\tfor v, _ in — blank index unnecessary, use for v in (INFO + auto-fix)")
    fmt.println("C037\tstyle\ttrailing return in void proc is redundant (INFO + auto-fix)")
    fmt.println("C201\tcorrectness\tError return value ignored (unchecked result)")
